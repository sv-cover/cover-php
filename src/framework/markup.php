...
function _markup_parse_images(&$markup, &$placeholders)
{
  ...
  while(preg_match('/\[img(?P<classes>(\.[a-z-]+)*)=(?P<url>.+?)\]/', $markup, $match)) {
    ...
    $placeholders[$placeholder] = sprintf(
      '<img class="%s" src="%s">',
...
{
  $count = 0;
  while(preg_match("/\[person( name=(?P<name>[^\]]+))?( position=(?P<position>[^\]]+))?\](?P<content>.+?)\[\/person\]/is", $markup, $match)) {
//    preg_match('/\[membersonly(=(?P<title>[^\]]+))?\](?P<content>.+?)\[\/membersonly\]/is', $markup, $match))
    $content = explode("\n", $match['content']);
    $placeholder = sprintf('#PERSON%d#', $count++);

    $placeholders[$placeholder] = '<div class="person notification"><div class="columns"><div class="column is-one-quarter">';

//		if ($match['image']) {
//			$placeholders[$placeholder] .= '<img src="' . $match['image'] . '" aria-hidden="true" class="person-image" width="128" />';
//		}
    if($match['name']) {
    ...
    $placeholders[$placeholder] .= '</div><div class="column">';

    foreach($content as $contentParagraph) {
      if($contentParagraph != '\n\n') {
        $placeholders[$placeholder] .= '<p>' . $contentParagraph . '</p>';
      }
    }

    $placeholders[$placeholder] .= '</div></div></div>';
...
function _markup_parse_emails(&$markup, &$placeholders)
{
  ...
  while(preg_match('/\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}\b/i', $markup, $match)) {
    ...
    $placeholders[$placeholder] = sprintf(
      '<a rel="nofollow" href="mailto:%s">%s</a>',
      rawurlencode($match[0]),
      markup_format_text($match[0])
    );

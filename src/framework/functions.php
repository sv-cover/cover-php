<?php
	...
	function run_view($name, $model = null, $iter = null, $params = null) {
		...
		try {
			...
			if (!_load_view("../public/views/$name.php"))
				throw new ViewNotFoundException(sprintf(__("The view %s could not be found"), $name));

			/* Locate the function */
			if (function_exists("view_$function"))
				return call_user_func("view_$function", $model, $iter, $params);
			else
				throw new ViewNotFoundException(sprintf(__("The view function %s in %s could not be found"), $function, $name));
		}
	}

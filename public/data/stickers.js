$(document).on('ready partial-content-loaded', function(e) {
	var ensureMapsIsLoaded = function(callback) {
		try {
			!!google.maps; // Assume this exists
		} catch (e) {
			var callback_hook = 'googleMapsCallbackHook';
			
			window[callback_hook] = function() {
				delete window[callback_hook];
				callback();
			};

			var el = document.createElement('script');
			el.src = 'https://maps.googleapis.com/maps/api/js?key=AIzaSyBN22N-bX3aSaGfy9w9-oeUsnFRlB-1FiI&callback=' + callback_hook + '&sensor=false';
			document.body.appendChild(el);

			return; // Don't fall out of this catch statement, go directly to jail, don't pass start, don't receive 200 dollars.
		}
